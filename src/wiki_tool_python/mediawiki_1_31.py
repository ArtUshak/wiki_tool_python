...
from wiki_tool_python.mediawiki import (
    CanNotDeleteError,
    MediaWikiAPI,
    MediaWikiAPIMiscError,
    PageProtectedError,
    StatusCodeError,
)
from wiki_tool_python.requests_wrapper import ThrottledSession
from wiki_tool_python.utils import HTTP_STATUS_OK

ParamsDict = dict[str, None | str | int]


class MediaWikiAPI1_31(MediaWikiAPI):  # noqa: N801
    ...
    def __init__(
        self, url: str, request_interval: float, user_agent: str
    ) -> None:
    ...
    def get_page_image_list(
        ...
        while i < len(page_ids):
            ...
            current_params["pageids"] = "|".join(list(map(str, page_ids_group)))
    ...
    def get_page(self, title: str) -> str:
        ...
        if r.status_code != HTTP_STATUS_OK:
            raise StatusCodeError(self.index_url, r.status_code)
    ...
    def get_deletedrevs_list(
        ...
        params: ParamsDict = {
            ...
            "adrnamespace": namespace,
            "adrdir": "newer",
            "adrlimit": limit,
            "adrprop": "ids|user|comment|content",
            "format": "json",
        }
        last_continue: dict[str, object] = {}
        ...
        while True:
            ...
            data = self.call_api(current_params)
            deletedrevs = data["query"]["alldeletedrevisions"]
    ...
    def upload_file(
        ...
        if r.status_code != HTTP_STATUS_OK:
            raise StatusCodeError(self.api_url, r.status_code)
        ...
        while True:
            ...
            if "error" in data:
                if data["error"]["code"] == "protectedpage":
                    raise PageProtectedError(data["error"])
                raise MediaWikiAPIMiscError(data["error"])
            backlinks = data["query"]["backlinks"]

            yield from backlinks
    ...
    def call_api(
        ...
        while True:
            ...
            if "error" in data:
                if (need_token and token_retry) and (
                ...
                if "code" in data["error"]:
                    if data["error"]["code"] == "cantdelete":
                        raise CanNotDeleteError(data["error"]["info"])
                    if data["error"]["code"] == "protectedpage":
                        raise PageProtectedError(data["error"])
